...
<script src="changecss.js"></script>
	...
	<meta name="twitter:site" content="">
	<meta property="og:site_name" content="">
	<meta property="og:title" content="[{{$oya[0][0]['no']}}] {{$oya[0][0]['sub']}} by {{$oya[0][0]['name']}} - {{$title}}">
	<meta property="og:type" content="article">
...
<br><center>
				...
				<form action="{{$self}}" method="post" enctype="multipart/form-data">
					<p>
						...
						<select name="shi">
							...
							@if($use_shi_painter)<option value="1" class="for_pc">Shi-Painter</option>
							@endif
							@if($use_chickenpaint)<option value="chicken">ChickenPaint</option>
							@endif
							@if ($use_klecks)<option value="klecks">Klecks</option>
							@endif
						</select>
						@else
						{{-- <!-- App that starts when the selection menu is not displayed --> --}}
						...
						@endif
						@if($anime)<label><input type="checkbox" value="true" name="anime" title="Save Playback" @if($animechk){{$animechk}}@endif>Save Playback</label>
								@endif
			...
			<div>
				...
				<form action="{{$self}}" method="post" enctype="multipart/form-data">
					...
					@if($resno)<input type="hidden" name="resto" value="{{$resno}}">
					@endif
					...
					<table>
						...
						hey
						<div class="g-recaptcha" data-sitekey="6LdKf4EqAAAAAPeoRkYDKu8-WFCRAzBX2phUtCn1"></div>
						<tr>
						...
						<tr>
							<td>Pass</td>
							<td><input class="form" type="password" name="pwd" value="" autocomplete="current-password"><small>(For editing and deleting)</small></td>
						</tr>
					</table>
					<ul>
						@if($upfile)
	...
	<main>
		<section>
			...
			<div class="thread">
				...
				{{-- 親記事のヘッダ --}}
				<h3>
					<img src="/bullets/icon_97_10.gif" alt=""> <span class="name"><a href="search.php?page=1&amp;imgsearch=on&amp;query={{$res['encoded_name']}}&amp;radio=2" target="_blank" rel="noopener">{{$res['name']}}</a></span><span class="trip">{{$res['trip']}}</span>
					{{$res['now']}}
					@if($res['id']) ID : {{$res['id']}}
					@endif
					@if($res['url']) <span class="url">[<a href="{{$res['url']}}" target="_blank" rel="nofollow noopener noreferrer">URL</a>]</span>
					@endif
				...
				<div class="res">
					{{-- 子レスヘッダ --}}
					<h4>
						<img src="{{$skindir}}img/comment.gif" alt="">
						<span class="name"><a href="search.php?page=1&amp;imgsearch=on&amp;query={{$res['encoded_name']}}&amp;radio=2" target="_blank" rel="noopener">{{$res['name']}}</a></span><span class="trip">{{$res['trip']}}</span> : {{$res['now']}}
						@if($res['id']) ID :
						{{$res['id']}}
						@endif
						@if($res['url']) <span class="url">[<a href="{{$res['url']}}" target="_blank" rel="nofollow noopener noreferrer">URL</a>]</span>
						@endif
						@if($res['updatemark']) {{$res['updatemark']}}
						@endif
					</h4>
					{{-- 子レスヘッダここまで --}}
					@endif
					...
					<div class="img_info_wrap">
						...
						({{$res['size_kb']}} KB)
						@if($res['thumb']) - Showing thumbnail -
						@endif
						@if($res['painttime']) PaintTime :
						{{$res['painttime']}}
						@endif
						<br>
					</div>
					<figure @if($res['w']>=750) style="float:none;margin-right:0"@endif>
							<a href="{{$res['src']}}" target="_blank" rel="noopener" class="luminous">
								<img src="{{$res['imgsrc']}}" alt="{{$res['sub']}} by {{$res['name']}}" title="{{$res['sub']}} by {{$res['name']}}" width="{{$res['w']}}" height="{{$res['h']}}" @if($i>4)loading="lazy"@endif>
							</a>
							<div align="center" style="padding-top:5px;">
								@if($res['continue']) <a href="{{$self}}?mode=continue&amp;no={{$res['continue']}}">*Continue</a>
								@endif
								@if($res['spch'])<span class="for_pc">
								@endif
								@if($res['pch']) <a href="{{$self}}?mode=openpch&amp;pch={{$res['pch']}}" target="_blank">*Replay</a>
								@endif
				...
				<div class="thfoot">
					...
					<span class="share_button">
						...
						<a target="_blank" class="fb btn" href="http://www.facebook.com/share.php?u={{$encoded_rooturl}}{{$encoded_self}}?res={{$ress[0]['encoded_no']}}"><span class="button"><img src="{{$skindir}}img/facebook.svg" alt="">
								Share</span></a>
					</span>
					@endif
					@if($notres)<span class="button"><a href="{{$self}}?res={{$ress[0]['no']}}"><img src="{{$skindir}}img/rep.svg" alt="">
						@if($ress[0]['disp_resbutton'])Reply
						@else
		...
		<div>
			...
			<nav>
				...
				<div class="pcdisp page">
					...
					@if($res_prev)<a href="{{$self}}?res={{$res_prev['no']}}">≪{{$res_prev['substr_sub']}}</a>
					@endif
					| <a href="{{$self2}}">Top</a> |
					@if($res_next)<a href="{{$self}}?res={{$res_next['no']}}">
					{{$res_next['substr_sub']}}≫</a>
					@endif
				</div>
				...
				<div class="view_other_works">
					@foreach($view_other_works as $view_other_work)<div><a href="{{$self}}?res={{$view_other_work['no']}}"><img src="{{$view_other_work['imgsrc']}}" alt="{{$view_other_work['sub']}} by {{$view_other_work['name']}}"
								title="{{$view_other_work['sub']}} by {{$view_other_work['name']}}" width="{{$view_other_work['w']}}" height="{{$view_other_work['h']}}" loading="lazy"></a></div>
					@endforeach
				</div>
				@endif
